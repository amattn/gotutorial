...
func TestAllShortlinks(t *testing.T) {

	final_url := test_server.URL + "/admin/list"
	res, err := http.Get(final_url)
	if err != nil {
		log.Fatal(3377573872, final_url, err)
	}

	assertEqual(t, 200, res.StatusCode, 0, final_url)

	body, _ := ioutil.ReadAll(res.Body)

	contains_amattn := strings.Contains(string(body), "amattn.com")

	assertEqual(t, true, contains_amattn, 1, final_url)
